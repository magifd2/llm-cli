...
### 🐛 バグ修正
*   **ストリーミング応答時のパニック**: `--stream` オプション使用時に発生していた「panic: close of closed channel」エラーを修正しました。`sync.Once`を使用して `responseChan` が一度だけクローズされるようにしました。

## v1.0.0 - 2025-08-12

### ✨ 機能
*   **設定ファイルパスオプション**: デフォルトの場所を上書きする、設定ファイルへのパスを指定するためのグローバルな `--config` (`-c`) フラグを導入しました。
*   **設定検証を伴うプロファイルチェックコマンドの強化**: `llm-cli profile check` コマンドが、プロバイダー固有の設定検証を実行するようになりました。
    *   新しい `llm.ConfigValidator` インターフェースを導入し、各LLMプロバイダーが実装しています。
    *   各プロバイダーの `ValidateConfig()` メソッドは、必須設定（例: `model`、`api-key`、`aws-region`、`project-id`、`credentials-file`）とその有効性をチェックします。
    *   `check` コマンドは詳細な検証エラーを報告するようになり、ユーザーが設定ミスを早期に特定し修正するのに役立ちます。

### ♻️ リファクタリング
*   **クレデンシャルファイルパスの解決**: `config.json` ファイルの場所に対する相対パスで `credentials-file` パスを解決し、`~`（チルダ）をユーザーのホームディレクトリに展開するロジックを実装しました。これにより、現在の作業ディレクトリに関係なく正しいパス解決が保証されます。
*   **プロバイダー設定検証インターフェース**: `llm.ConfigValidator` インターフェースを定義し、すべてのLLMプロバイダー（`ollama`、`openai`、`openai2`、`bedrock`、`vertexai`、`vertexai2`、`mock`）で `ValidateConfig()` メソッドを実装し、設定検証ロジックを一元化および標準化しました。

## v0.0.12 - 2025-08-08

### ✨ 機能
*   **高度なOpenAI互換プロバイダー (`openai2`)**: OpenAI互換API（特にLM Studio）向けに、動的で柔軟なモデル解決機能を持つ新しい `openai2` プロバイダーを導入しました。
    *   `model` 設定に、優先順位を指定するカンマ区切りのリスト（例: `"my-model,auto"`）を使用できます。
    *   プロバイダーはサーバーの `/v1/models` エンドポイントをクエリし、優先順位リストに一致する最初の利用可能なモデルを使用します。これにより、プロファイルを頻繁に編集する必要がなくなり、ワークフローが簡素化されます。

### ♻️ リファクタリング
*   **プロバイダーのパッケージ分離**: `internal/llm` パッケージを、各プロバイダーが自己完結したサブパッケージ（例: `internal/llm/openai`）に配置されるように再構築しました。この大規模なリファクタリングにより、厳格なモジュールの独立性が確保され、保守性が大幅に向上し、堅牢なユニットテストへの道が開かれました。


## v0.0.11 - 2025-08-08

### ✨ 機能
*   **Vertex AI プロバイダーの強化 (`vertexai2`)**: Vertex AI SDKがネイティブでシステムプロンプトをサポートしていない問題に対する、より堅牢なワークアラウンドを提供する新しい `vertexai2` プロバイダーを追加しました。このプロバイダーは、チャット履歴の最初のターンとしてシステムプロンプトを注入することで会話のコンテキストを事前設定し、より一貫したモデルの振る舞いを実現します。
...
### 📝 Documentation
*   **開発経緯の更新**: `docs/development_logs/dev_log-2025-08-02.md` にVertex AI SDK移行の詳細な経緯とSDKの現状に関する記述を追加しました。
...
### 📝 Documentation
*   **開発ログと開発計画の追加**: プロジェクトの経緯と今後の計画を記録するため、`docs/development_logs/` を作成しました。
