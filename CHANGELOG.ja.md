...
*   **Bedrock認証情報ハンドリングの強化**:
    *   Bedrockの認証情報読み込みを外部JSONファイルに対応させ、セキュリティとプロファイル間での再利用性を向上させました。
    *   `Profile` 構造体内の `CredentialsFile` フィールドを汎用化し、AWSとGCP両方の認証情報ファイルパスを扱えるようにして冗長性を削減しました。
    *   `CredentialsFile` のパス解決を実装し、`~` の展開と絶対パスへの変換を行うことで、セキュリティと明確性を向上させました。
    *   `profile show` コマンドを更新し、`CredentialsFile` の解決済み絶対パスを表示するようにして、ユーザーへの透明性を高めました。
    *   `profile add` および `profile set` コマンドを、統合された `credentials-file` オプションを使用するように調整しました。
