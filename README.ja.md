...
*   **マルチプロバイダー対応**: Ollama、LM Studio（およびその他のOpenAI互換API）、Amazon Bedrock、Google Cloud Vertex AI (vertexai, vertexai2)とシームレスに連携します。
...
`llm-cli` は、設定ファイルへのパスを指定するために、すべてのコマンドで使用できるグローバルな `--config` フラグをサポートしています。これにより、デフォルトの場所を上書きできます。

| フラグ     | 短縮形 | 説明                                                                 |
| -------- | ------ | -------------------------------------------------------------------- |
| `--config` | `-c`   | 設定ファイルへのパス（デフォルトは `$HOME/.config/llm-cli/config.json`）。 |
...
この方法では、`llm-cli` バイナリをビルドし、指定されたディレクトリにインストールします。
...
`llm-cli` をアンインストールするには、インストール時に使用したのと同じ `PREFIX` を指定して `make uninstall` を使用します。
...
**パス表記に関する注意:** `~` (チルダ) 文字は、ユーザーのホームディレクトリを表す一般的な省略形です。`llm-cli` は、サポートされているすべてのオペレーティングシステム（Linux、macOS、Windows）で、これを適切なホームディレクトリパスに正しく展開し、クロスプラットフォームの互換性を確保します。
...
`llm-cli` のすべての設定は、単一の設定ファイルで管理されます。デフォルトでは、このファイルは `~/.config/llm-cli/config.json` にあります。ただし、グローバルな `--config` (`-c`) フラグを使用してカスタムパスを指定できます。`llm-cli profile edit` でこのファイルを直接編集することもできますが、`profile` サブコマンド群を使用することが推奨されます。
...
**注意:** `credentials-file` には、AWS認証情報JSONファイルへのパスを `~` を含む形式、絶対パス、または `llm-cli` 設定ファイルからの相対パスで指定できます。`~` はホームディレクトリに展開され、相対パスは `config.json` ファイルの場所に基づいて解決されます。JSONファイルには `aws_access_key_id` と `aws_secret_access_key` フィールドが含まれている必要があります。例：
...
**注意:** `credentials-file` には、サービスアカウントキーのJSONファイルへのパスを `~` を含む形式、絶対パス、または `llm-cli` 設定ファイルからの相対パスで指定してください。`~` はホームディレクトリに展開され、相対パスは `config.json` ファイルの場所に基づいて解決されます。このフィールドは、AWS Bedrockの認証情報ファイルにも使用されるようになりました。

**必要なIAMロール:**
サービスアカウントには、Vertex AIモデルを呼び出す権限が必要です。
*   `Vertex AI ユーザー` ロール

**システムプロンプトの扱い: SDKの制限に対するワークアラウンド**

`llm-cli` が利用している Google Cloud Vertex AI SDK には、専用の「システムプロンプト」機能がネイティブサポートされていません。この制限を回避するため、`llm-cli` はそれぞれ異なるワークアラウンドを実装した2つのプロバイダーを提供します。

*   **`vertexai` (標準ワークアラウンド):** このプロバイダーは、システムプロンプトの内容を会話の最初のユーザーメッセージとして送信し、その直後に実際のユーザープロンプトを送信するという基本的なワークアラウンドを実装しています。

*   **`vertexai2` (拡張ワークアラウンド):** このプロバイダーは、チャット履歴を事前設定することにより、より堅牢なワークアラウンドを採用しています。最初のターンでユーザーがシステムプロンプトを指示し、2ターン目でモデルがそれを承諾する（例：「OK」と応答する）という2ターンの会話履歴を構築します。この手法により、後続のユーザープロンプトに対するモデルの振る舞いのコンテキストが明確に確立され、より一貫した結果が期待できます。この拡張ワークアラウンドを利用するには、プロファイルの `provider` を `vertexai2` に設定してください。
...
旧バージョンからアップグレードしたユーザーや、すべてのプロファイルが最新のデフォルト制限設定を持つことを確認したい場合は、`profile check` コマンドを使用できます。
```bash
llm-cli profile check
```
このコマンドは、すべてのプロファイルを検査し、デフォルトのゼロ値（古いバージョンからのものか、明示的に設定されていないことを示す）である `limits` 設定を更新するかどうかをユーザーに確認します。また、変更を行う前に `config.json` のタイムスタンプ付きバックアップを作成し、データの安全性を確保します。
